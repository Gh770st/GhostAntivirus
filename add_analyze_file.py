...
# Find where to insert the analyze_file method
insert_point = content.find("    /// Get current status")

if insert_point != -1:
    # Insert the analyze_file method
    new_method = '''    /// Analyze file using AI/ML models
    pub async fn analyze_file(&self, features: &FileFeatures) -> Result<Option<AnalysisResult>> {
        if !self.enabled {
            return Ok(None);
        }
...
'''
    ...
    print("✅ Added analyze_file method to AIIntegration")
else:
    print("❌ Could not find insertion point for analyze_file")